...
const statutColor = { confirmé:'var(--olive)', 'en attente':'#dd2b21', 'devis envoyé':'#878f90', livré:'var(--olive)', expédié:'#4A7A9B', 'en préparation':'#dd2b21' }
...
export default function Admin() {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.4}}
      style={{ background:'#222325', minHeight:'100vh', paddingTop:'90px' }}>
      ...
      <div style={{ background:'rgba(255,255,255,.02)', borderBottom:'1px solid rgba(255,255,255,.06)', padding:'20px 48px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'16px' }}>
          {alertes > 0 && (
            <div style={{ padding:'6px 14px', background:'rgba(221,43,33,.15)', border:'1px solid rgba(221,43,33,.3)', fontFamily:'var(--sans)', fontSize:'10px', color:'#FF7070', letterSpacing:'.1em' }}>
              ⚠ {alertes} stock{alertes>1?'s':''} en alerte
            </div>
          )}
          <div style={{ width:'36px', height:'36px', borderRadius:'50%', background:'rgba(221,43,33,.15)', border:'1px solid rgba(221,43,33,.3)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--serif)', fontSize:'14px', color:'var(--gold)' }}>CF</div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {tab==='stocks' && (
            <motion.div key="stocks" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid rgba(255,255,255,.06)', overflow:'hidden' }}>
                {/* Table header */}
                <div style={{ display:'grid', gridTemplateColumns:'2.5fr 1fr 1fr 1fr 1fr 140px', background:'rgba(221,43,33,.06)', borderBottom:'1px solid rgba(221,43,33,.12)', padding:'12px 24px' }}>
                ...
                {stock.map((w, i) => (
                  <div key={w.id} style={{ display:'grid', gridTemplateColumns:'2.5fr 1fr 1fr 1fr 1fr 140px', padding:'16px 24px', borderBottom: i<stock.length-1?'1px solid rgba(255,255,255,.04)':'none', alignItems:'center', transition:'background .2s' }}
                    onMouseEnter={e=>e.currentTarget.style.background='rgba(221,43,33,.03)'}
                    ...
                    <div>
                      {editing===w.id ? (
                        <input type="number" value={editVal} onChange={e=>setEditVal(e.target.value)}
                          ...
                          autoFocus
                          style={{ width:'72px', padding:'4px 8px', background:'rgba(221,43,33,.1)', border:'1px solid var(--gold)', color:'var(--gold)', fontFamily:'var(--sans)', fontSize:'13px', outline:'none' }}/>
                      ) : (
                    ...
                    <div style={{ display:'flex', gap:'8px' }}>
                      {editing===w.id ? (
                        ...
                        <button onClick={()=>{setEditing(w.id);setEditVal(String(w.stock))}} style={{ padding:'6px 14px', background:'transparent', border:'1px solid rgba(221,43,33,.25)', color:'rgba(221,43,33,.7)', fontFamily:'var(--sans)', fontSize:'9px', letterSpacing:'.12em', textTransform:'uppercase', cursor:'pointer', transition:'all .3s' }}
                          onMouseEnter={e=>{e.target.style.borderColor='var(--gold)';e.target.style.color='var(--gold)'}}
                          onMouseLeave={e=>{e.target.style.borderColor='rgba(221,43,33,.25)';e.target.style.color='rgba(221,43,33,.7)'}}>
                          Modifier
                        </button>
                      )}
                    </div>
          ...
          {tab==='reservations' && (
            <motion.div key="resa" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid rgba(255,255,255,.06)', overflow:'hidden' }}>
                <div style={{ display:'grid', gridTemplateColumns:'80px 1.5fr 1.5fr 1fr 80px 100px 120px', background:'rgba(221,43,33,.06)', borderBottom:'1px solid rgba(221,43,33,.12)', padding:'12px 24px' }}>
          ...
          {tab==='commandes' && (
            <motion.div key="cmd" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div style={{ background:'rgba(255,255,255,.02)', border:'1px solid rgba(255,255,255,.06)', overflow:'hidden' }}>
                <div style={{ display:'grid', gridTemplateColumns:'80px 1.5fr 80px 1fr 2fr 120px 120px', background:'rgba(221,43,33,.06)', borderBottom:'1px solid rgba(221,43,33,.12)', padding:'12px 24px' }}>
                ...
                {commandes.map((c,i)=>(
                  <div key={c.id} style={{ display:'grid', gridTemplateColumns:'80px 1.5fr 80px 1fr 2fr 120px 120px', padding:'16px 24px', borderBottom: i<commandes.length-1?'1px solid rgba(255,255,255,.04)':'none', alignItems:'center', transition:'background .2s' }}
                    ...
                    <div>
                      <span style={{ padding:'3px 8px', background: c.type==='B2B'?'rgba(74,93,35,.3)':c.type==='Club'?'rgba(221,43,33,.3)':'rgba(255,255,255,.05)', color: c.type==='B2B'?'#8FAF45':c.type==='Club'?'#C8506A':'rgba(245,245,243,.4)', fontFamily:'var(--sans)', fontSize:'8px', letterSpacing:'.1em', textTransform:'uppercase' }}>{c.type}</span>
                    </div>
