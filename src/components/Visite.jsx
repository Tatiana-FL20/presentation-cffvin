...
export default function Visite() {
  return (
    <section style={{ background: 'transparent', padding: '120px 48px', position: 'relative', overflow: 'hidden' }}>

      {/* Ambient */}
      <div style={{ position: 'absolute', bottom: '-200px', left: '-200px', width: '600px', height: '600px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(221,43,33,0.09) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', top: '-100px', right: '-100px', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(221,43,33,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .9 }}
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, rgba(34,35,37,.7) 0%, rgba(34,35,37,.2) 60%, rgba(34,35,37,.5) 100%)' }} />
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '0 64px' }}>
            <div>
              ...
              <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(38px,5vw,68px)', fontWeight: 300, color: 'var(--black)', lineHeight: 1.05 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'end', marginBottom: '80px' }}>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .8 }}>
            <p style={{ fontFamily: 'var(--serif)', fontSize: '18px', fontStyle: 'italic', color: 'rgba(34,35,37,.5)', lineHeight: 1.8, marginBottom: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
              {[
                ...
                <motion.div key={item.titre}
                  ...
                  style={{ display: 'flex', gap: '16px', alignItems: 'flex-start', padding: '20px 0', borderBottom: i < 3 ? '1px solid rgba(34,35,37,.07)' : 'none' }}>
                  <div style={{ color: 'var(--gold)', flexShrink: 0, marginTop: '2px', opacity: .8 }}>{item.icon}</div>
                  <div>
                    <div style={{ fontFamily: 'var(--sans)', fontSize: '11px', fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--black)', marginBottom: '4px' }}>{item.titre}</div>
                    <div style={{ fontFamily: 'var(--serif)', fontSize: '14px', fontStyle: 'italic', color: 'rgba(34,35,37,.4)', lineHeight: 1.6 }}>{item.desc}</div>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .8, delay: .15 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {/* Adresse */}
              <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', paddingBottom: '28px', borderBottom: '1px solid rgba(34,35,37,.09)' }}>
                <div style={{ color: 'rgba(221,43,33,.7)', flexShrink: 0, marginTop: '2px' }}>
                ...
                <div>
                  <div style={{ fontFamily: 'var(--sans)', fontSize: '9px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(34,35,37,.25)', marginBottom: '8px' }}>Adresse</div>
                  <div style={{ fontFamily: 'var(--sans)', fontSize: '14px', color: 'rgba(34,35,37,.8)', lineHeight: 1.7 }}>
              ...
              {/* Horaires */}
              <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', padding: '28px 0', borderBottom: '1px solid rgba(34,35,37,.09)' }}>
                <div style={{ color: 'rgba(221,43,33,.7)', flexShrink: 0, marginTop: '2px' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
                ...
                <div>
                  <div style={{ fontFamily: 'var(--sans)', fontSize: '9px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(34,35,37,.25)', marginBottom: '8px' }}>Horaires</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <div style={{ display: 'flex', gap: '16px', fontFamily: 'var(--sans)', fontSize: '13px' }}>
                      <span style={{ color: 'rgba(34,35,37,.4)', minWidth: '80px' }}>Lun – Ven</span>
                      <span style={{ color: 'rgba(34,35,37,.8)' }}>08h00–11h30 · 13h00–16h00</span>
                    </div>
                    <div style={{ display: 'flex', gap: '16px', fontFamily: 'var(--sans)', fontSize: '13px' }}>
                      <span style={{ color: 'rgba(34,35,37,.4)', minWidth: '80px' }}>Samedi</span>
                      <span style={{ color: 'rgba(34,35,37,.8)' }}>08h30–11h00</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', paddingTop: '28px' }}>
                ...
                <div>
                  <div style={{ fontFamily: 'var(--sans)', fontSize: '9px', letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(34,35,37,.25)', marginBottom: '8px' }}>Contact direct</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <a href="tel:+261342232604" style={{ fontFamily: 'var(--sans)', fontSize: '14px', color: 'rgba(34,35,37,.8)', textDecoration: 'none', transition: 'color .3s' }}
                      onMouseEnter={e => e.target.style.color = 'var(--gold)'}
                      onMouseLeave={e => e.target.style.color = 'rgba(34,35,37,.8)'}>034 22 326 04</a>
                    <a href="mailto:info@chanfoui.mg" style={{ fontFamily: 'var(--sans)', fontSize: '14px', color: 'rgba(34,35,37,.8)', textDecoration: 'none', transition: 'color .3s' }}
                      onMouseEnter={e => e.target.style.color = 'var(--gold)'}
                      onMouseLeave={e => e.target.style.color = 'rgba(34,35,37,.8)'}>info@chanfoui.mg</a>
                  </div>
                </div>
              </div>
            </div>
        ...
        <motion.div initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true }} transition={{ duration: 1.2 }}
          style={{ height: '1px', background: 'linear-gradient(90deg, transparent, rgba(221,43,33,.3), transparent)', marginBottom: '64px', transformOrigin: 'left' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: 'rgba(221,43,33,.1)' }}>
          {privileges.map((p, i) => (
            <motion.div key={p.titre}
              ...
              style={{ background: 'var(--gray)', padding: '36px 32px', position: 'relative', overflow: 'hidden' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(221,43,33,.04)'}
              onMouseLeave={e => e.currentTarget.style.background = 'var(--gray)'}>
              <div style={{ position: 'absolute', top: '24px', right: '24px', fontFamily: 'var(--serif)', fontSize: '48px', fontWeight: 300, color: 'rgba(221,43,33,.06)', lineHeight: 1, userSelect: 'none' }}>
                {String(i + 1).padStart(2, '0')}
              </div>
              <div style={{ width: '24px', height: '1px', background: 'var(--gold)', marginBottom: '20px' }} />
              <h3 style={{ fontFamily: 'var(--serif)', fontSize: '20px', fontWeight: 300, color: 'var(--black)', marginBottom: '10px', lineHeight: 1.2 }}>{p.titre}</h3>
              <p style={{ fontFamily: 'var(--sans)', fontSize: '13px', color: 'rgba(34,35,37,.4)', lineHeight: 1.7, margin: 0 }}>{p.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .7, delay: .2 }}
          style={{ marginTop: '64px', padding: '56px', border: '1px solid rgba(221,43,33,.2)', background: 'linear-gradient(135deg, rgba(221,43,33,.08), transparent 60%)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          <p style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(20px,3vw,32px)', fontWeight: 300, fontStyle: 'italic', color: 'var(--black)', lineHeight: 1.4, marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => navigate('b2b')}
              style={{ padding: '16px 48px', background: 'transparent', border: '1px solid rgba(221,43,33,.35)', color: 'rgba(34,35,37,.7)', fontFamily: 'var(--sans)', fontSize: '10px', fontWeight: 300, letterSpacing: '.2em', textTransform: 'uppercase', cursor: 'pointer', transition: 'all .3s' }}
              onMouseEnter={e => { e.target.style.borderColor = 'var(--gold)'; e.target.style.color = 'var(--gold)' }}
              onMouseLeave={e => { e.target.style.borderColor = 'rgba(221,43,33,.35)'; e.target.style.color = 'rgba(34,35,37,.7)' }}
            >Espace B2B</button>
          </div>
        </motion.div>
