...
export default function WineClub() {
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.6}}
      style={{ background:'var(--black)', minHeight:'100vh', paddingTop:'120px', paddingBottom:'120px' }}>
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'0 48px' }}>

        <button onClick={()=>navigate('home')} style={{ fontFamily:'var(--sans)', fontSize:'10px', letterSpacing:'.2em', textTransform:'uppercase', color:'rgba(245,245,243,.4)', background:'none', border:'none', cursor:'pointer', marginBottom:'60px', padding:0 }}
        ...
        <motion.div initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} transition={{duration:.8}} style={{ textAlign:'center', marginBottom:'80px' }}>
          ...
          <h1 style={{ fontFamily:'var(--serif)', fontSize:'clamp(44px,6vw,80px)', fontWeight:300, color:'var(--white)', lineHeight:1.05, marginBottom:'20px' }}>
            <em style={{ fontStyle:'italic' }}>Wine</em> Club
          </h1>
          <div style={{ width:'48px', height:'1px', background:'var(--gold)', margin:'0 auto 28px' }}/>
          <p style={{ fontFamily:'var(--serif)', fontSize:'19px', fontStyle:'italic', color:'rgba(245,245,243,.5)', maxWidth:'500px', margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'24px' }}>
          {wineClub.map((club, i) => (
            <motion.div key={club.id} initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} transition={{delay:i*.15, duration:.7}}
              style={{ position:'relative', border:`1px solid ${club.recommended ? 'var(--gold)' : 'rgba(255,255,255,.08)'}`, padding:'40px 32px', background: club.recommended ? 'rgba(200,169,106,.05)' : 'rgba(255,255,255,.02)', animation: club.recommended ? 'glow 3s ease-in-out infinite' : 'none' }}>
              ...
              <h2 style={{ fontFamily:'var(--serif)', fontSize:'40px', fontWeight:300, color:'var(--white)', marginBottom:'8px' }}>{club.nom}</h2>
              <div style={{ fontFamily:'var(--serif)', fontSize:'28px', color:'var(--gold)', marginBottom:'32px' }}>{club.prix}</div>

              <div style={{ borderTop:'1px solid rgba(255,255,255,.07)', paddingTop:'28px', marginBottom:'36px' }}>
                {club.avantages.map(a=>(
                  <div key={a} style={{ display:'flex', alignItems:'flex-start', gap:'10px', marginBottom:'12px' }}>
                    ...
                    <span style={{ fontFamily:'var(--sans)', fontSize:'13px', color:'rgba(245,245,243,.65)', lineHeight:1.5 }}>{a}</span>
                  </div>
                ))}
              </div>
