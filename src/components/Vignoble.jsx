  ...
  return (
    <section id="vignoble" ref={ref} style={{ position:'relative', overflow:'hidden' }}>
      ...
      <motion.div style={{ y, position:'relative', height:'60vh', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom, rgba(34,35,37,.45), rgba(34,35,37,.15) 50%, rgba(34,35,37,.65))' }}/>
        <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:'12px' }}>
          <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:1}}>
            ...
            <h2 style={{ fontFamily:'var(--serif)', fontSize:'clamp(44px,7vw,88px)', fontWeight:300, color:'var(--black)', textAlign:'center', lineHeight:.95 }}>
      ...
      {/* Stats */}
      <div style={{ background:'transparent', display:'grid', gridTemplateColumns:'repeat(4,1fr)', borderTop:'1px solid rgba(221,43,33,.15)' }}>
        {stats.map((s,i)=>(
          <motion.div key={s.label} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.1}}
            style={{ padding:'40px 24px', textAlign:'center', borderRight: i<3 ? '1px solid rgba(221,43,33,.1)' : 'none' }}>
            <div style={{ fontFamily:'var(--serif)', fontSize:'48px', fontWeight:300, color:'var(--gold)', lineHeight:1 }}>{s.val}</div>
            <div style={{ fontFamily:'var(--sans)', fontSize:'9px', letterSpacing:'.2em', textTransform:'uppercase', color:'rgba(34,35,37,.35)', marginTop:'8px' }}>{s.label}</div>
          </motion.div>
        ))}
      </div>

      {/* Galerie */}
      <div style={{ background:'transparent', padding:'52px 80px', position:'relative' }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
          ...
          <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.8}}
            ...
            <h2 style={{ fontFamily:'var(--serif)', fontSize:'clamp(28px,4vw,48px)', fontWeight:300, color:'var(--black)', lineHeight:1.05 }}>
