    ...
    <motion.div layout initial={{opacity:0,y:32}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-16}} transition={{duration:.5}}
      ...
      <div style={{ background:'#ffffff', border:'1px solid rgba(34,35,37,.08)', padding:'40px 28px', transition:'all .4s cubic-bezier(.16,1,.3,1)', transform: hovered ? 'translateY(-6px)' : 'translateY(0)', boxShadow: hovered ? '0 12px 40px rgba(34,35,37,.08)' : '0 2px 8px rgba(34,35,37,.04)' }}>
        ...
        <div style={{ position:'absolute', top:'16px', right:'16px', display:'flex', gap:'6px', flexDirection:'column', alignItems:'flex-end' }}>
          ...
          {wine.badge && !wine.premium && <span style={{ padding:'3px 10px', background:'var(--bordeaux)', color:'var(--black)', fontFamily:'var(--sans)', fontSize:'8px', fontWeight:500, letterSpacing:'.15em', textTransform:'uppercase' }}>{wine.badge}</span>}
        </div>
        ...
        <h3 style={{ fontFamily:'var(--serif)', fontSize:'22px', fontWeight:400, color:'var(--black)', marginBottom:'12px', lineHeight:1.2 }}>{wine.nom}</h3>

        <motion.div animate={{ opacity: hovered ? 1 : 0, height: hovered ? 'auto' : 0 }} transition={{ duration:.3 }} style={{ overflow:'hidden' }}>
          <div style={{ borderTop:'1px solid rgba(34,35,37,.09)', paddingTop:'16px', display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'12px' }}>
    ...
    <section id="vins" style={{ background:'transparent', padding:'120px 48px' }}>
      <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
        ...
        <motion.div initial={{opacity:0,y:40}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.9}}
          ...
          <p style={{ fontFamily:'var(--serif)', fontSize:'19px', fontStyle:'italic', color:'rgba(34,35,37,.45)', maxWidth:'480px', margin:'0 auto' }}>
