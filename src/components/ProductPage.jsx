...
export default function ProductPage({ wine }) {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.6}}
      style={{ background:'transparent', minHeight:'100vh', paddingTop:'100px' }}>

      {/* Back */}
      <div style={{ maxWidth:'1200px', margin:'0 auto', padding:'0 48px' }}>
        <button onClick={()=>navigate('home')} style={{ fontFamily:'var(--sans)', fontSize:'10px', letterSpacing:'.2em', textTransform:'uppercase', color:'rgba(34,35,37,.4)', background:'none', border:'none', cursor:'pointer', marginBottom:'48px', display:'flex', alignItems:'center', gap:'8px', padding:0 }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'80px', alignItems:'start' }}>
            ...
            <div style={{ position:'relative', display:'flex', justifyContent:'center', padding:'60px 0' }}>
              ...
              {wineImages[`../assets/vins/${wine.id}.png`]?.default
                ...
                : <svg width="120" height="300" viewBox="0 0 120 300" fill="none" style={{ animation:'float 5s ease-in-out infinite', filter:`drop-shadow(0 0 40px ${wine.color}55)` }}>
                    ...
                    <path d="M44 80h32v80h-32z" fill="rgba(34,35,37,.08)"/>
                  </svg>
              }
            </div>
            {/* Arom wheel */}
          ...
          <div style={{ paddingTop:'40px' }}>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.1}}>
              ...
              <h1 style={{ fontFamily:'var(--serif)', fontSize:'clamp(40px,5vw,64px)', fontWeight:300, color:'var(--black)', lineHeight:1.1, marginBottom:'28px' }}>{wine.nom}</h1>
              <div style={{ width:'48px', height:'1px', background:'var(--gold)', marginBottom:'28px' }}/>
              <p style={{ fontFamily:'var(--serif)', fontSize:'18px', fontStyle:'italic', color:'rgba(34,35,37,.65)', lineHeight:1.85, marginBottom:'40px' }}>{wine.desc}</p>
            </motion.div>

            {/* Accords */}
            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.4}}>
              <div style={{ fontFamily:'var(--sans)', fontSize:'9px', letterSpacing:'.25em', textTransform:'uppercase', color:'rgba(34,35,37,.35)', marginBottom:'12px' }}>Accords Parfaits</div>
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'48px' }}>
                {wine.accord.map(a=>(
                  <span key={a} style={{ padding:'6px 14px', border:'1px solid rgba(221,43,33,.25)', color:'rgba(34,35,37,.6)', fontFamily:'var(--sans)', fontSize:'11px' }}>{a}</span>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.5}}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'16px', marginBottom:'20px' }}>
                <div style={{ display:'flex', alignItems:'center', border:'1px solid rgba(34,35,37,.14)' }}>
                  <button onClick={()=>setQty(q=>Math.max(1,q-1))} style={{ width:'44px', height:'44px', background:'none', border:'none', color:'var(--black)', fontFamily:'var(--sans)', fontSize:'16px', cursor:'pointer' }}>−</button>
                  <div style={{ width:'44px', textAlign:'center', fontFamily:'var(--sans)', fontSize:'14px', color:'var(--black)' }}>{qty}</div>
                  <button onClick={()=>setQty(q=>q+1)} style={{ width:'44px', height:'44px', background:'none', border:'none', color:'var(--black)', fontFamily:'var(--sans)', fontSize:'16px', cursor:'pointer' }}>+</button>
                </div>
              ...
              <p style={{ fontFamily:'var(--sans)', fontSize:'11px', color:'rgba(34,35,37,.25)', letterSpacing:'.05em' }}>Livraison gratuite dès 6 bouteilles · Toute Madagascar</p>
            </motion.div>
          </div>
        </div>
      </div>
